...
public class sem6task1 {
    ...
    private static void printNotebooks(Set<Notebook> notebooks, Map<String, String> filter) {
        ...
        for (Notebook n : notebooks) {
            if (filter(n, filter)) {
                String s = String.format("S/N:%12s: ОЗУ(Гб):%d, диск(Гб):%d, экран(дюйм):%d, ОС:%s, цвет:%s", n.getSerialNumber(), n.getRamSizeGb(), n.getDiskSizeGb(), n.getScreenSizeInch(), n.getOs(), n.getColor());
                forPrint.add(s);
            }
        }
    ...
    public static Set<Notebook> initSet() {
        ...
        //
        set.add(new Notebook("W123456", 15, 8, 512, OS_WINDOWS_10, "PINK"));
        set.add(new Notebook("L234567", 17, 8, 128, OS_LINUX_UBUNTU, "BLACK"));
        set.add(new Notebook("L234567WT", 17, 4, 4000, OS_LINUX_UBUNTU, "WHITE"));
        set.add(new Notebook("W123456BLU", 15, 16, 1024, OS_WINDOWS_10, "BLUE"));
